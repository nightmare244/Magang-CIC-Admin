    ...
    <div v-else class="space-y-6">
      <div v-if="izinList && izinList.length > 0" class="grid grid-cols-1 gap-6">
        <IzinCard
          ...
          @detail="goToDetail"
          @updateStatus="handleStatusRequest" 
        />
      </div>
      ...
      <div v-if="hasPagination" class="flex flex-col sm:flex-row justify-between items-center pt-8 border-t border-gray-100 dark:border-gray-800 gap-6">
        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Navigasi Halaman Operasional</p>
        <div class="flex gap-3">
          <button @click="fetchData(rawResponse.prev_page_url)" :disabled="!rawResponse.prev_page_url" class="btn-pagination-eco">
            <ChevronLeft class="w-4 h-4 mr-2" /> Sebelumnya
          </button>
          <button @click="fetchData(rawResponse.next_page_url)" :disabled="!rawResponse.next_page_url" class="btn-pagination-eco">
...
import { FileCheck, ClipboardX, ChevronLeft, ChevronRight } from 'lucide-vue-next';
...
const goToDetail = (id) => {
  router.push({ name: 'admin.izin.detail', params: { id: id } });
};

// Menangkap request dari IzinCard
const handleStatusRequest = (data) => {
  // Cari objek izin lengkap berdasarkan ID
  selectedIzin.value = izinList.value.find(i => i.id === data.id);
...
// Dipanggil setelah API di dalam Modal sukses
...
const fetchData = async (url = '/admin/persetujuan-izin') => {
  ...
  try {
    ...
    console.error("Gagal sinkronisasi data API.");
  } finally {
...
.card-eco-premium { @apply transition-all duration-500 hover:shadow-2xl hover:-translate-y-1; }
