<template>
  <div class="p-4 md:p-8 max-w-full mx-auto animate-fade-in space-y-8 bg-[#f9fafb] dark:bg-[#0a0c0a] min-h-screen text-slate-800 dark:text-slate-200 font-poppins">
    ...
    <header class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 border-b border-gray-100 dark:border-gray-800 pb-8">
      <div class="flex items-center gap-5">
        ...
        <div>
          ...
          <p class="text-xs font-medium text-slate-400 mt-1">
            Registrasi unit kerja atau divisi baru ke dalam sistem operasional.
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2 items-end">
        <Transition name="slide-fade">
          <div v-if="apiError && !Object.keys(errors).length" class="flex items-center gap-3 px-4 py-2 bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800 rounded-xl">
        ...
        <Transition name="slide-fade">
          <div v-if="successMessage" class="flex items-center gap-3 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-xl animate-bounce">
    ...
    <form @submit.prevent="save" class="max-w-3xl mx-auto space-y-8 font-poppins">
      <div class="card-eco p-8 bg-white/50 backdrop-blur-sm space-y-8 shadow-xl border-none">
        ...
        <div class="space-y-3">
          ...
          <div class="relative group">
            ...
            <input 
              ...
              required 
              placeholder="Contoh: Pengembangan Teknologi"
            />
          </div>
          <Transition name="slide-fade">
            <p v-if="errors.nama_departemen" class="text-[10px] font-bold text-rose-500 uppercase mt-2 ml-1 italic">
        ...
        <div class="space-y-3">
          ...
          <textarea 
            ...
            placeholder="Jelaskan peran atau tanggung jawab departemen ini secara rincian..."
          ></textarea>
          <Transition name="slide-fade">
            <p v-if="errors.deskripsi" class="text-[10px] font-bold text-rose-500 uppercase mt-2 ml-1 italic">
              {{ errors.deskripsi[0] }}
            </p>
          </Transition>
        </div>
      ...
      <div class="flex flex-col sm:flex-row justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
        <button 
          type="button" 
        ...
        >
          ...
          {{ loading ? 'Memproses...' : 'Daftarkan Departemen' }}
        </button>
      </div>
    </form>
  </div>
...
const save = async () => {
  ...
  try {
    await api.post('/admin/departemens', form);
    ...
    if (error.response?.status === 422) {
      ...
      apiError.value = 'DATA TIDAK VALID';
    } else {
      apiError.value = error.response?.data?.message || 'GANGGUAN SISTEM';
    }
  } finally {
    loading.value = false;
  }
