        ...
        >
          ...
          <div class="p-8 pt-6">
            <div class="flex flex-col items-center text-center mb-6">
              <div class="w-20 h-20 bg-rose-50 dark:bg-rose-900/20 rounded-[2rem] flex items-center justify-center mb-4 text-rose-600 border border-rose-100 dark:border-rose-800/50 shadow-inner">
              ...
              <p class="kpi-label-small !text-rose-600 mt-2">Otoritas Penolakan Aset</p>
            </div>

            <div class="space-y-6">
              <div class="bg-slate-50 dark:bg-white/5 p-5 rounded-3xl w-full border border-slate-100 dark:border-white/5">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 text-left">Personel Pengaju:</p>
                <p class="text-sm font-bold text-slate-800 dark:text-white text-left italic">
                   {{ peminjaman?.user?.name || 'Karyawan' }}
                </p>
              </div>

              <div class="space-y-2">
                <label class="kpi-label-small !text-slate-400 ml-1">Alasan Penolakan Operasional <span class="text-rose-500">*</span></label>
                <textarea
                  v-model="alasan"
                  rows="3"
                  class="input-field-eco-textarea"
                  placeholder="Berikan alasan logis penolakan aset..."
                  required
                ></textarea>
            ...
            <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <button
                ...
                class="btn-reject-modal-eco order-1 sm:order-2"
              >
                <span v-if="!isProcessing" class="flex items-center justify-center">
                  <X class="w-4 h-4 mr-2" /> Konfirmasi Tolak
                </span>
          ...
          <div class="px-8 py-5 bg-slate-50 dark:bg-white/[0.02] border-t border-gray-100 dark:border-gray-800 flex flex-col items-center gap-1">
            <p class="text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.3em]">ECO Asset Management System</p>
...
const handleReject = async () => {
  ...
  try {
    await api.put(`/admin/persetujuan-peminjaman/${props.peminjaman.id}/reject`, { 
